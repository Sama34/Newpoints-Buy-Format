...
function newpoints_buyformat_info()
{
	...
	$info = array(
		...
		'author'		=> 'Omar G. & Edzon Ordaz',
		'authorsite'	=> 'http://mundomybb.com',
		'version'		=> '1.0',
		'compatibility'	=> '16*'
	);
    ...
    if(newpoints_buyformat_is_installed() && is_array($plugins) && is_array($plugins['active']) && $plugins['active']['newpoints_buyformat'] && $mybb->input['module'] != 'newpoints-settings')
    {
        ...
        $info["description"] .= "<br /><a href=\"{$editurl}\">Apply Edits</a>. | <a href=\"{$undourl}\">Undo Edits</a>.";
    }
...
function newpoints_buyformat_activate()
{
	...
	// Add our settings.
	newpoints_add_setting('newpoints_buyformat_power', 'newpoints_buyformat', $lang->npbf_s_power, $lang->npbf_s_power_d, 'yesno', '0', 1);
...
function newpoints_buyformat_install()
{
	...
	if(!$db->table_exists("newpoints_buyformat"))
	{
		$db->write_query("CREATE TABLE `".TABLE_PREFIX."newpoints_buyformat` (
			...
			`usergroups` text NOT NULL default '',
			PRIMARY KEY (`sid`)
			) ENGINE=MyISAM;"
		);
	}
